...
function hackingmg(success) {
  if (NeedToDoEverything && !ilkMinigameDone && success) {
    $(".mainapp, .appinside").hide();
    post("https://echo_computerhack/close", {});
    post("https://echo_computerhack/NotifyResmon", { notify: "You've completed the first hack, access the menu again and finish the second hack" });
    ilkMinigameDone = true;
  } else {
    if (success) {
      post("https://echo_computerhack/endTask", { success: true });
      $(".mainapp, .appinside").hide();
      post("https://echo_computerhack/close", {});
    } else {
      if (hackmgsayac === Number(randomnumberssans)) {
        post("https://echo_computerhack/endTask", { success: false });
        post("https://echo_computerhack/close", {});
        $(".mainapp, .appinside").hide();
        hackmgsayac = 1;
      } else {
